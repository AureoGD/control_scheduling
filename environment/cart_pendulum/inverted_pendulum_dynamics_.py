...
class InvePendulum():
    ...
    def __init__(self, dt=0.002):
        ...
        # System constraints
        self.f_max = 4  # Max force (N) 6.5 stabilize VF
        self.v_max = 3  # Max velocity (m/s)
    ...
    #         # Clamp cart accel to zero, pendulum evolves under gravity only (point mass):
    #         ddx = 0.0
    #         # From angular equation with ddx=0: m*l*dda = m*g*sin(a)  =>  dda = (g/l) * sin(a)
    #         dda = (self.g * sin(a)) / self.l

    #     else:
    #         # ---- Point-mass A matrix ----
    #         # Linear (cart):   mc*ddx + (m*l*cos a)*dda = F - b_r*dx + (m*l)*da^2*sin a
    #         # Angular (scaled): (m*cos a)*ddx + (m*l)*dda = m*g*sin a
    #         a_11 = self.mc
    #         a_12 = self.mr * self.l * cos(a)
    #         a_21 = self.mr * cos(a)
    #         a_22 = self.mr * self.l
    #         A = np.array([[a_11, a_12], [a_21, a_22]], dtype=float)

    #         # ---- Point-mass B vector ----
    #         b_1 = effective_force - self.br * effective_dx + (self.mr * self.l) * (da**2) * sin(a)
    #         b_2 = self.mr * self.g * sin(a)
    #         B = np.array([b_1, b_2], dtype=float)

    #         try:
    #             ddx, dda = np.linalg.solve(A, B)
    ...
    #     # ---- Enforce cart position limits ----
    #     prev_x = self.x
    #     self.x = np.clip(self.x, -self.x_max, self.x_max)
    #     if (self.x == self.x_max and dx > 0) or (self.x == -self.x_max and dx < 0):
    #         self.dx = 0.0
