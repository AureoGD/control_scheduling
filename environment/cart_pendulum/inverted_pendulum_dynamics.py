...
class InvePendulum():
    ...
    def __init__(self, dt=0.001, disturbance=False, soft_wall=True, noise = False):
        """
    ...
    def step_sim(self, force):
        ...
        for _ in range(10):
            ...
            if self.soft_wall:
                ...
                # For hard walls, completely stop the cart at boundaries
                if at_positive_limit or at_negative_limit:
                    # Stop the cart completely at the wall
                    self.dx = 0.0
                    self.x = np.clip(self.x, -self.x_max, self.x_max)
                    # Only allow forces that pull away from the wall
                    if (at_positive_limit and force > 0) or (at_negative_limit and force < 0):
                        force = 0
                    return self.get_state()

            # Calculate dynamics (only if not at hard wall boundary)
            I = self.J + self.mr * self.l**2
            Phi = I * (self.mc + self.mr) - (self.mr * self.l * cos(a))**2
            
            f2 = (-self.mr**2 * self.l**2 * da**2 * sin(a) * cos(a) + 
                (self.mr + self.mc) * self.mr * self.g * self.l * sin(a) + 
                self.mr * self.l * self.br * dx * cos(a) - 
                self.d * (self.mc + self.mr) * da)
            
            g2 = -self.mr * self.l * cos(a)
            
            f1 = (I * self.mr * self.l * sin(a) * da**2 - I * self.br * dx - 
                self.mr**2 * self.l**2 * self.g * cos(a) * sin(a) + 
                self.mr * self.l * self.d * cos(a) * da)
            
            g1 = I
            
            f_total = force + f_dis + f_wall
            ...
            if self.noise:
                self.ddan = random.uniform(-20, 20)
                self.ddxn = random.uniform(-10, 10)
            else:
                self.ddan = 0
                self.ddxn = 0

            # Update states
            self.dx += (ddx + self.ddxn) * self.dt
            self.x += self.dx * self.dt
            self.da += (dda + self.ddan) * self.dt
            self.a += self.da * self.dt

            # Apply constraints
            if not self.soft_wall:
                self.x = np.clip(self.x, -self.x_max, self.x_max)
